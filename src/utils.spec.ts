...
import { pathSet, pathKeysSet } from './utils';
...
describe('pathKeysSet() tests', () => {
  it('Update single path', () => {
    const person = generatePerson();
    const update = pathKeysSet(person, ['age'], 33);
  ...
  it('Update array path', () => {
    const person = generatePerson();
    const update = pathKeysSet(person, ['favoriteFood', 1], 'Meatballs');
  ...
  it('Update deeply nested path', () => {
    const person = generatePerson();
    const update = pathKeysSet(person, ['addresses', 'primary', 'city'], 'Chicago');
